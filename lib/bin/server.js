...
module.exports = async function server (opts = {}) {
  const keyFilename = path.resolve(opts.f || path.join(constants.dir, 'id_server'))
  ...
  if (firewallEnabled && !fs.existsSync(firewallFilename)) {
    console.log('Notice: Creating default firewall', crayon.green(firewallFilename))
