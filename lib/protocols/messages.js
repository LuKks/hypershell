...
shell.spawn = {
  preencode (state, s) {
    ...
    c.uint.preencode(state, s.width || 0)
    c.uint.preencode(state, s.height || 0)
  },
  encode (state, s) {
    c.string.encode(state, s.command || '')
    stringArray.encode(state, s.args || [])
    c.uint.encode(state, s.width || 0)
    c.uint.encode(state, s.height || 0)
  },
  decode (state) {
    return {
...
const admin = exports.admin = {}

admin.invite = {
  requestEncoding: {
    preencode (state, v) {
      c.uint.preencode(state, v.expiry)
    },
    encode (state, v) {
      c.uint.encode(state, v.expiry)
    },
    decode (state) {
      return {
  ...
  responseEncoding: {
    preencode (state, v) {
      c.buffer.preencode(state, v)
    },
    encode (state, v) {
      c.buffer.encode(state, v)
    },
    decode (state) {
      return c.buffer.decode(state)
    }
  }
}
