...
module.exports = class Tunnel {
  ...
  async remote (remoteAddress, localAddress) {
    ...
    this.wireServerStart.send(serverInfo)
    ...
    const onWireClose = async () => {
      const wait = withResolvers()
      const time = 500 * Math.min(retryCount++, 5)
    ...
    this._events.on('close', onWireClose)
    ...
    return {
      close: async () => {
        ...
        this._events.off('close', onWireClose)
        ...
        const serverClosed = this._wait(serverId)
